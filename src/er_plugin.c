...

{
    ...
    {
        cpl_msg_error(er_func, "cannot get current working directory");
        move_err = 64;
            ...
            else
            {
                ...
                if(status == 0)
                    cpl_msg_warning (er_func,
                                   "Product final output path file already exists and will be overwritten (%s)",
                                   output_name);
                status = rename(cpp, output_name);
                if(status != 0)
                {
                    if(errno == EXDEV)
                    {
                        status = fileutils_move(cpp, output_name);
                        if(status != 0)
                        {
                            cpl_frameset_iterator_delete(it);
                            cpl_msg_error (er_func,
                                   "Unable to move product file to final output path in different filesystem (%s)",
                                   output_name);
                            move_err = 66;
                            goto report_products;
                        }
                    ...
                    else if(errno == EACCES)
                    {
                        cpl_frameset_iterator_delete(it);
                        cpl_msg_error (er_func,
                               "Permission denied in product final output directory  (%s)",
                               output_dir);
                        move_err = 67;
                        goto report_products;
                    }
                    else if(errno == EROFS)
                    {
                        cpl_frameset_iterator_delete(it);
                        cpl_msg_error (er_func,
                               "Read-only filesystem in product final output directory  (%s)",
                               output_dir);
                        move_err = 68;
                        goto report_products;
                    }
